...
loadTasks();
// Se llama a la función para cargar las tareas almacenadas en Local Storage y mostrarlas en el DOM.
...
// Función para actualizar las tareas en Local Storage
function updateLocalStorage() {
  const tasks = Array.from(taskList.querySelectorAll("li")).map(
    (li) => li.firstChild.textContent
  );
  // Se obtienen todas las tareas actuales del DOM y se convierten en un arreglo.
...
// Obtener el tema actual desde Local Storage
const currentTheme = localStorage.getItem("theme");
console.log(currentTheme);
// Se imprime el tema actual en la consola para depuración.

if (currentTheme === "dark") {
  document.body.classList.add("dark-theme");
  // Si el tema guardado es "dark", se aplica la clase "dark-theme" al cuerpo.
}

// Manejar el evento de clic en el botón de alternar tema
themeToggleButton.addEventListener("click", () => {
  document.body.classList.toggle("dark-theme");
  // Alterna la clase "dark-theme" en el cuerpo.

  const theme = document.body.classList.contains("dark-theme")
    ? "dark"
    : "light";
  // Determina el tema actual (oscuro o claro).

  localStorage.setItem("theme", theme);
  // Guarda el tema actual en Local Storage.
});
